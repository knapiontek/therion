...
if __name__ == '__main__':
    ...
    points = np.zeros((n ** 2 + n1 ** 2, 3), dtype=np.float64)
    ...
    for x in range(n1):
        for y in range(n1):
            points[offset1 + n1 * x + y] = [w + x, h + 2 * h * y, 0]

    # lines

    lines = np.zeros((4 * n1 ** 2 + n1 * n + n2 * n1, 2), dtype=np.int32)
    ...
    offset3 = offset2 + n1 * n

    for x in range(n2):
        for y in range(n1):
            print(offset3 + n1 * x + y)
            lines[offset3 + n1 * x + y] = [offset1 + n1 * x + y, offset1 + n1 * (x + 1) + y]
