...
public class Test {
	public static void run() {
		try {
			...
			X509Certificate rootCertificate = keyGen.getSelfCertificate(new X500Name("CN=ROOT"),
					(long) 365 * 24 * 60 * 60);
			...
			X509Certificate middleCertificate = keyGen1.getSelfCertificate(new X500Name("CN=MIDDLE"),
					(long) 365 * 24 * 60 * 60);
			...
			X509Certificate topCertificate = keyGen2.getSelfCertificate(new X500Name("CN=TOP"),
					(long) 365 * 24 * 60 * 60);
	...
	private static void storeKeyAndCertificateChain(String alias, char[] password, String keystore, Key key,
	...
	private static X509Certificate createSignedCertificate(X509Certificate cetrificate,
		...
		try {
			...
			// No need to add the BasicContraint for leaf certificate
