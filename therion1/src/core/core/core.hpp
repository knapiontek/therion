...
#define b2r(value) { value, sizeof(value) }
#define c2r(value) { (core::uint8*)value, sizeof(value) - 1 }
#define s2r(value) { value.data(), value.size() }
...
public:
	virtual void throw_alloc_exception()
	{

	}
	virtual void print_assert(const char* file_name, int line_no, const char* content)
	{
...
inline void print_assert(bool expr, const char* file_name, int line_no, const char* content)
{
	if(!expr)
		Handler::handler()->print_assert(file_name, line_no, content);
}

#define assert(expr) print_assert((expr), __FILE__, __LINE__, __STRING(expr))
#define verify(expr) print_assert((expr), __FILE__, __LINE__, __STRING(expr))
