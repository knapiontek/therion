...
template<typename Key, typename Value, class TreeIndex = Index>
class TreeMap
{
	struct Node;
	struct Page;
...
public:
	...
	Find find(const Key& key)
	{
		...
		while(pos)
		{
			int32 dir = the_index.compare((Key&)key, pos->key);
			if(dir)
			...
			while(pos)
			{
				...
				else
				{
					...
					if(pos != tail)
					{
						...
						while(tail_pos != tail)
						{
							int32 dir = the_index.compare(tail->key, tail_pos->key);
							if(dir)
							{
