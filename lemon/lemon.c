...
struct lemon {
  ...
  char *outdir;            /* Name of the output directory */
...
char **argv;
{
  ...
  if( OptNArgs()!=2 ){
    fprintf(stderr,"Exactly filename and output directory arguments are required.\n");
    exit(1);
  }
  lem.errorcnt = 0;
...
struct pstate {
  ...
  int preccnt;           /* Assign this precedence to decl arguments */
...
char *suffix;
{
  ...
  name = malloc( strlen(lemp->outdir) + strlen(lemp->filename) + strlen(suffix) + 6 );
  if( name==0 ){
    fprintf(stderr,"Can't allocate space for a filename.\n");
    exit(1);
  }
  strcpy(name,lemp->outdir);
  strcat(name,"/");
...
int mhflag;     /* Output in makeheaders format if true */
{
  ...
  if( mhflag ){
    char *name = file_makename(lemp, ".hpp");
