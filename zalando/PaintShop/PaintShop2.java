/*
	Optimized for small number of colors!
	...
	run:
		javac PaintShop2.java
		java PaintShop2 <sample.txt>
*/

import java.io.*;
import java.util.*;

public class PaintShop2
{
	// helps to find out whether matte has to be used when glossy is fully ruled out
	static class CustomerInfo
	{
		public int matte = -1; // index of unique matte
	...
	static void run(Scanner scanner)
	{
		...
		for (int c = 1; c <= C; c++)
		{
			...
			// search-index tuned for small number of colors which is used later to remove impossible glossy
			CustomerInfoList[] glossyCustomerInfoListArr = new CustomerInfoList[N];
			...
			for (int m = 1; m <= M; m++)
			{
				...
				for (int t = 1; t <= T; t++)
				{
					int X = scanner.nextInt() - 1; // color index adapted for arrays
					...
					if (Y == 0)
					{
						...
						CustomerInfoList customerInfoList = glossyCustomerInfoListArr[X];
						if (customerInfoList == null)
						{
							customerInfoList = new CustomerInfoList();
							glossyCustomerInfoListArr[X] = customerInfoList;
						}
			...
			if (analyze(matteOnlyList, glossyCustomerInfoListArr))
			{
	...
	static boolean analyze(List<Integer> matteOnlyList, CustomerInfoList[] glossyCustomerInfoListArr)
	{
		for (int i = 0; i < matteOnlyList.size(); i++) // matteOnlyList modified in the inner loop
		{
			...
			CustomerInfoList customerInfoList = glossyCustomerInfoListArr[matteColor];
			glossyCustomerInfoListArr[matteColor] = null;
			if (customerInfoList != null)
			{
				for (CustomerInfo customerInfo : customerInfoList)
				{
					...
					{
						if (customerInfo.matte != -1) // new matte comes up, re-analyze it as well
						{
